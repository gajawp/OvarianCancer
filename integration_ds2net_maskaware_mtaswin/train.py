...
from integration_ds2net_maskaware_mtaswin.dataset import (
...
def plot_history(history):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Mask-Aware MTA-Swin Loss")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    figure.savefig(
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Macro F1")
    plt.title("Mask-Aware MTA-Swin Macro F1")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    figure.savefig(
...
def main():
    ...
    print("=" * 78)
    print("DS2NET RGB ROI + EXPLICIT MASK MTA-SWIN TRAINING")
    print("=" * 78)
