...
def extract_final_logits(model_output):
    """
    Extract the final segmentation logits from U-Net++.

    U-Net++ commonly returns multiple deep-supervision
    outputs. The last output is used as the final prediction.
    """
    ...
    if isinstance(model_output, (tuple, list)):
        if len(model_output) == 0:
            raise ValueError(
                "U-Net++ returned an empty tuple or list."
            )
        ...
        for key in possible_keys:
            if key in model_output:
                ...
                if isinstance(value, (tuple, list)):
                    if len(value) == 0:
                        raise ValueError(
                            "U-Net++ output dictionary "
                            "contains an empty output list."
                        )
...
def train_one_epoch(
    ...
    -------
    tuple
        Average loss, Dice score, and IoU score.
    """
    ...
    for batch in progress_bar:
        ...
        # Apply the loss to all deep-supervision outputs.
...
def save_checkpoint(
    ...
    """
    Save U-Net++ weights and training metadata.
    """
...
def main():
    """
    Train U-Net++ using only the official training split.
    ...
    checkpoint_path = (
        config.UNETPLUSPLUS_MODEL_PATH
    )
    ...
    print(
        "\nRun unetplusplus.evaluate to calculate final "
