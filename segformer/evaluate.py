...
def save_qualitative_result(image, target_mask, predicted_mask, index, save_directory):
    ...
    plt.imshow(image)
    plt.imshow(predicted_mask, cmap="jet", alpha=0.35)
    plt.axis("off")

    plt.tight_layout()
    output_path = save_directory / f"segformer_result_{index:03d}.png"
...
def main():
    ...
    validation_loader = create_validation_loader(config, batch_size=1)
    ...
    save_directory = config.SEGFORMER_RESULTS_DIR
    ...
    with torch.no_grad():
        for index, (image, mask) in enumerate(
            tqdm(validation_loader, desc="Evaluating SegFormer")
        ):
            ...
            logits = model(image)
            prediction = (
                torch.sigmoid(logits) > config.PREDICTION_THRESHOLD
            ).float()

            prediction_numpy = prediction.cpu().numpy().squeeze()
            ...
            metric_values["hausdorff_distance"].append(
                hausdorff_distance(prediction_numpy, mask_numpy)
            )
    ...
    print(f"Hausdorff Dist. : {average_metrics['hausdorff_distance']:.4f}")

    append_result(
        csv_path=config.MODEL_COMPARISON_PATH,
        model_name="SegFormer",
        metrics=average_metrics,
    )

    print("\nComparison table updated:", config.MODEL_COMPARISON_PATH)
