...
def preprocess_image(image_path):
    """Read and convert one image into a model input tensor."""
    ...
    normalized_image = normalized_image.transpose(
        2,
        0,
        1,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a DS2Net segmentation mask."
    )
    ...
    parser.add_argument(
        "--output",
        default="ds2net_prediction.png",
        help="Path where the prediction figure will be saved.",
    )
    ...
    predicted_mask = (
        predicted_mask.cpu().squeeze().numpy()
    )
