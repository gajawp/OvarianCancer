...
def extract_final_logits(model_output):
    """
    Extract the final segmentation logits from DSNet output.
    ...
    if isinstance(model_output, (tuple, list)):
        if len(model_output) == 0:
            raise ValueError(
                "DSNet returned an empty tuple or list."
            )
    ...
    if isinstance(model_output, dict):
        ...
        raise KeyError(
            ...
            "DSNet output dictionary."
        )

    raise TypeError(
        "Unsupported DSNet output type: "
...
def train_one_epoch(
    ...
    if total_samples == 0:
        raise RuntimeError(
            "The DSNet training DataLoader contained "
            "no samples."
        )
...
def save_checkpoint(
    ...
    """
    Save DSNet weights and training metadata.
    """
...
def main():
    """
    Train DSNet using only the official training split.
    ...
    print("=" * 70)
    print("DSNet Segmentation Training")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    print("DSNet training completed")
    print("=" * 70)
    ...
    print(
        "\nRun dsnet.evaluate to calculate final metrics "
